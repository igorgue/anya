...
class CompactAgent:
        ...
        try:
            ...
            def analyze_context(conversation_text: str) -> str:
                ...
                for line in lines:
                    ...
                    if '@' in line and any(ext in line for ext in ['.py', '.js', '.lua', '.md', '.txt']):
                        file_references.append(line.strip())
                    
                    # Look for action items
                    if any(word in line.lower() for word in ['todo:', 'fix:', 'implement', 'add', 'create']):
                        active_tasks.append(line.strip())
                        
                    # Look for decisions
                    if any(word in line.lower() for word in ['decided', 'agreed', 'conclusion', 'final']):
                ...
                return '\n'.join(analysis) if analysis else "No specific key elements identified."
    ...
    def _create_validate_tool(self):
        ...
        try:
            ...
            def validate_summary(original: str, summary: str) -> str:
                ...
                if missing_files:
                    validation.append(f"⚠️  Missing file references: {', '.join(missing_files)}")
                else:
    ...
    def compact_conversation(self, conversation_history: List[Dict], target_tokens: Optional[int] = None) -> str:
        ...
        try:
            ...
            # Determine target
            if target_tokens:
                target_prompt = f"Compact the conversation to approximately {target_tokens} tokens."
            else:
    ...
    def compact_with_instructions(self, conversation_history: List[Dict], 
        ...
        if not instruction_agent:
            self.logger.warning("Instruction-aware agent not available, using enhanced fallback")
    ...
    def _fallback_compaction_with_instructions(self, conversation_history: List[Dict], 
        ...
        try:
            ...
            for line in lines:
                ...
                if any(marker in line for marker in ['## ', '### ', 'TODO:', 'FIXME:', '@', '```']):
                    should_keep = True
            ...
            if target_tokens:
                ...
                if len(filtered_text) > target_chars:
                    ...
                    filtered_lines = filtered_lines[:int(target_chars / len('\n'.join(filtered_lines[:1]) + '\n'))]
            ...
            if not filtered_lines:
                result_lines.append("*No content matched the specified instructions. Original conversation preserved in compacted form.*")
                result_lines.append(conversation_text[:1000] + "..." if len(conversation_text) > 1000 else conversation_text)
    ...
    def _create_instruction_aware_analyze_tool(self):
        ...
        try:
            ...
            def analyze_with_instructions(conversation_text: str, user_instructions: str) -> str:
                """
                ...
                if any(word in instruction_lower for word in ['keep', 'preserve', 'focus on']):
                ...
                if any(word in instruction_lower for word in ['remove', 'avoid', 'exclude']):
                ...
                for line in lines:
                    ...
                    for word in avoid_words:
                        if word in instruction_lower:
                            ...
                            if topic_idx >= 0:
                                after = instruction_lower[topic_idx + len(word):].strip()
...
class ContextAnalyzer:
    ...
    def extract_key_elements(self, conversation: List[Dict]) -> Dict[str, Any]:
        ...
        task_indicators = ['todo:', 'fix:', 'implement', 'add', 'create', 'update', 'modify']
    ...
    def calculate_importance_score(self, element: str) -> float:
        ...
        if '@' in element and any(ext in element for ext in ['.py', '.js', '.lua', '.md']):
            score += 0.3
        
        # Higher score for action items
        if any(word in element.lower() for word in ['implement', 'fix', 'add', 'create']):
            score += 0.2
